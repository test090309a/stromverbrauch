...
# #
# 12.09.2025 - Werte Positionen geändert oder gelöscht.
#####################################################################################################
...
GRUNDGEBUEHR = 3.50  # Euro pro Monat
...
def calculate_total_cost(
    total_consumption_kwh, price_per_kwh, grundgebuehr=0.0, months=0
):
...
def print_power():
    ...
    if power:
        ...
        # Berechnung der Gesamtkosten nach neuer Formel
        # Gesamtverbrauch in kWh:
        total_consumption_kwh = total_power_consumption / 1000
        # Preis pro kWh:
        price_per_kwh = current_price if current_price else 0.33
        # Monate seit Start:
        months = max(1, int((current_time - start_time) / (30 * 24 * 3600)))
        # Gesamtkosten inkl. Grundgebühr:
        total_cost_with_base = calculate_total_cost(
            total_consumption_kwh, price_per_kwh, GRUNDGEBUEHR, months
        )
        ...
        if current_time - last_log_time >= 900:  # 900 Sekunden = 15 Minuten
            ...
            logging.info(
                f"Gesamtkosten der Sitzung: {total_cost_with_base:.3f} Euro | "
            ...
            last_log_time = (
                current_time  # Aktualisiere den Zeitpunkt der letzten Protokollierung
            )

        # Einrücken der big_digits-Ausgabe um 4 Leerzeichen
        # big_digits_output = big_digits(f"{total_cost_with_base:.3f}€")
        big_digits_output = big_digits(f"{current_price:.3f}€")
        # output = "" + ""
...
#             f"\n    {Fore.YELLOW}Zeit: {format_duration(current_time - start_time)}"
  #       )
        ...
        # output += f"    {Fore.YELLOW}Grundgebühr: {GRUNDGEBUEHR:.2f} €/Monat, Monate: {months}\n"
        # output += f"    {Fore.YELLOW}Gesamtverbrauch: {total_consumption_kwh:.3f} kWh\n"
...
def main():
    ...
    start_time = time.time()
    remaining_time = 0.01  # Startwert für den Countdown
    ...
    start_time = time.time()
    remaining_time = 0  # Startwert für den Countdown
