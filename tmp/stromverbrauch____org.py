        ...
        if response.status_code == 200:
            ...
            current_price_data = next(
                (
                    ...
                    if datetime.fromisoformat(item["date"]).replace(tzinfo=None)
                    <= now
                    < datetime.fromisoformat(item["date"]).replace(tzinfo=None)
...
# Funktion, um die Gesamtkosten zu berechnen (inkl. Grundgebühr)
def calculate_total_cost(
    total_consumption_kwh, price_per_kwh, grundgebuehr=0.0, monate=1
):
    return (total_consumption_kwh * price_per_kwh) + (grundgebuehr * monate)
...
def print_power():
    ...
    # Durchschnittsberechnung wie gehabt
    ...
    if power:
        ...
        # Berechne den Gesamtverbrauch in kWh
        total_power_consumption_kwh = total_power_consumption / 1000
        # Setze den Preis pro kWh
        preis_pro_kwh = current_price if current_price else 0.33
        # Grundgebühr und Monate (hier als Beispiel, ggf. anpassen)
        grundgebuehr = 5.0  # Euro pro Monat, anpassen!
        monate = max(1, int((current_time - start_time) / (30 * 24 * 3600)))
        # Gesamtkosten nach neuer Formel
        total_cost_since_start = calculate_total_cost(
            total_power_consumption_kwh, preis_pro_kwh, grundgebuehr, monate
        )

        # Logging wie gehabt
        if current_time - last_log_time >= 900:
            logging.info(
                f"Gesamtkosten der Sitzung: {total_cost_since_start:.3f} Euro | "
                f"Aktueller Verbrauch: {power:.2f} Watt | "
                f"Laufzeit: {format_duration(current_time - start_time)} | "
                f"Aktueller Strompreis: {preis_pro_kwh:.3f} €/kWh"
            )
        ...
        output += f"    {Fore.YELLOW + Style.NORMAL}Preis: {preis_pro_kwh:.3f} EUR/kWh {Fore.LIGHTBLACK_EX + Style.DIM}\n    Quelle:{price_provider}\n"
        output += f"    {Fore.YELLOW}Grundgebühr: {grundgebuehr:.2f} €/Monat für {monate} Monat(e)\n"
        ...
        sys.stdout.write(
            "\033[H\033[J"
        )  # Cursor an den Anfang des Bildschirms setzen und Bildschirm löschen
...
def main():
    ...
    print(
        Fore.YELLOW
        + f"    \033]8;;file://{log_file}\033\\               L I N K               \033]8;;\033\\"
    )
