...
# Funktion zum Ausblenden des Cursors
def hide_cursor():
    if os.name == 'nt':
        ci = ctypes.windll.kernel32.GetConsoleCursorInfo
        sci = ctypes.windll.kernel32.SetConsoleCursorInfo
        h = ctypes.windll.kernel32.GetStdHandle(-11)
        ci(h, ctypes.byref(ctypes.c_long(0)))
    else:
        sys.stdout.write("\033[?25l")
        sys.stdout.flush()

# Funktion zum Einblenden des Cursors
def show_cursor():
    if os.name == 'nt':
        ci = ctypes.windll.kernel32.GetConsoleCursorInfo
        sci = ctypes.windll.kernel32.SetConsoleCursorInfo
        h = ctypes.windll.kernel32.GetStdHandle(-11)
        ci(h, ctypes.byref(ctypes.c_long(1)))
    else:
...
def print_power():
    ...
    if power:
        ...
        output = "".join(Fore.GREEN + line + "\n" for line in big_digits(f"{total_cost_since_start:.3f}€")) # Kommastellen definieren.
        ...
        peak_output += f"{Style.RESET_ALL}" # Alle Font Stile löschen.
...
def main():
    get_current_price()
    hide_cursor()  # Cursor ausblenden
    try:
        while True:
            print_power()
            time.sleep(1)
    except KeyboardInterrupt:
        show_cursor()  # Cursor wieder einblenden
